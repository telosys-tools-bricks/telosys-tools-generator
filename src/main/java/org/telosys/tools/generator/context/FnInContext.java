...
public class FnInContext {
	...
	@VelocityMethod(
		text={	
			"Returns a list of strings with the name of each attribute" },
		example={ 
			"$fn.attributeNames( $entity.attributes )",
			"$fn.attributeNames( $entity.keyAttributes )"},
		parameters = { 
			"attributes : list of attributes" },
		since = "4.3.0"
			)
	public List<String> attributeNames( List<AttributeInContext> attributes ) {
		if ( attributes != null ) {
			...
			return Collections.emptyList(); // not supposed to happen 
		}
	}
	...
	/**
	 * Throws a GenerationCancellationException 
	 * Intentionally not documented (just for tests)
	 * @param message
	...
	@VelocityMethod(
		text={	
			"Returns a string constructed by joining all strings of the given collection after applying a transformation.",
			"A replacement based on the given template is applied to each string.",
			"The variable name in the template is always '%' "
		},
		parameters = { 
			"collection : the collection of strings ",
			"separator  : the separator string (can be an empty string) ",
			"template   : the template to use to apply the transformation",
		},
		example = {
			"#set ( $v = $fn.joinWithTransformation( $items, \",\", \"foo.%\" ) ) ",
			"#set ( $v = $fn.joinWithTransformation( $items, \",\", \"%=foo.%\"  ) ",
		},
		since = "4.3.0"
		)
	public String joinWithTransformation(Collection<String> collection, String separator, String template) {
		return joinWithTransformation(collection, separator, template, "%");
	}
	...
	@VelocityMethod(
		...
		parameters = { 
			...
			"separator  : the separator string (can be an empty string) ",
			"template   : the template to use to apply the transformation",
			"variable   : the variable name used in the template"
		},
		example = {
			"#set ( $v = $fn.joinWithTransformation( $items, \", \", \"foo.$name\", \"$name\" ) ) ",
			"#set ( $v = $fn.joinWithTransformation( $items, \",\", \"${x}=foo.${x}\", \"${x}\") ) ",
		},
		since = "4.3.0"
		)
	...
	@VelocityMethod(
		text={	
			"Returns a string consisting of N tab characters"
			},
		parameters = { 
			"n : the number of tab characters expected" 
			}
	)
